...
- シーンは5〜8個程度
- 各シーンのナレーションは自然な日本語で
- text_overlay は必ずナレーション（narration）と一言一句同じ文章にしてください。要約や言い換えは一切禁止です。絶対に「シーン1」などの文字は含めないこと。
- 【重要】かぎかっこの後に句読点の「。」は絶対に付けないでください（例：「○○○」。 ではなく 「○○○」 とすること）。
...
- 【重要】過去のありふれたアイデアや完全な空想（例：人の頭の上に数字が見える等）は絶対に避けてください。実在する地名、建物、歴史上の出来事、実在の人物、または現存する物を題材に組み込み、視聴者が引き込まれるようなリアリティのある台本を作成してください（※都市伝説や怪談などのテーマの場合は、本当にありそうな不気味なエピソードにしてください）。
- 【重要】台本の中で「これは100％創作ですが」「ここから話すのはフィクションです」といったメタな発言（前置き）は絶対にしないでください。事実や実際の体験談のように自然に語ってください。
"""
